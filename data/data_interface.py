...
class DataInterface(LightningDataModule):
    def __init__(self, dataset:str, dataset_params:dict, num_workers:int = 8, batch_size:int = 3, data_rate:list = [0.9, 0.1, 0.0]):
        super().__init__()
    ...
    def setup(self, stage:typing.Literal["fit", "validate", "test", "predict"] = "fit", auto_split_dataset:bool = True):
        if stage == "fit":
    ...
    def predict_dataloader(self):
        return torch.utils.data.DataLoader(self.predictset, batch_size = 1,  num_workers = self.num_workers, shuffle = False)